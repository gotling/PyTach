		...
		% for command in activity["activities"]:
            ...
            <button class="pure-button pure-u-1-2 pure-u-md-1-3 pure-u-lg-1-4" onclick='post("{{ url('activity', activity=command_path) }}")'>{{command["description"] if "description" in command else command["name"]}}</button>
		% end
		</div>
